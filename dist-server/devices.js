...
export async function runDeviceCommand(dev, command, timeoutMs = 30000) {
    if (dev.type === "ssh") {
...
function runSSH(dev, command, timeoutMs = 30000) {
    ...
    return new Promise((resolve) => {
        ...
        catch { } resolve({ stdout: "", stderr: "timeout", exitCode: -1 }); }, timeoutMs);
    });
}
// Test connection to a device
export async function testDevice(dev) {
    try {
        if (dev.type === "ssh") {
...
export async function deviceScreenshot(dev) {
    if (dev.type === "ssh") {
