    ...
    try {
        ...
        if (method === "POST" && path === "/api/engines/build") {
            ...
            try {
                ...
                const wikiSummary = await fetch(`https://en.wikipedia.org/api/rest_v1/page/summary/${encodeURIComponent(subject)}`, { signal: AbortSignal.timeout(10000) });
                const wikiData = await wikiSummary.json();
                const wikiExtract = wikiData.extract || "";
                const wikiTitle = wikiData.title || subject;
                // Get more content
                const wikiContent = await fetch(`https://en.wikipedia.org/w/api.php?action=query&prop=extracts&explaintext=true&exsectionformat=plain&titles=${encodeURIComponent(wikiTitle)}&format=json`, { signal: AbortSignal.timeout(10000) });
                const contentData = await wikiContent.json();
                const pages = contentData.query?.pages ?? {};
                const fullExtract = Object.values(pages)[0]?.extract || wikiExtract;
