...
import { getDevice, runDeviceCommand, deviceScreenshot } from "./devices.js";
...
export function getToolsForBot(opts) {
...
export function createToolExecutor(cfg, botDeviceId) {
    ...
    executors.execute_command = async (args) => {
        ...
        if (devId) {
            ...
            if (!dev)
                return "Error: device not found: " + devId;
        ...
        try {
            ...
            return new Promise((resolve) => {
                ...
                const timer = setTimeout(() => {
                    ...
                    if (stdout)
                        resolve(stdout + (stderr ? nl + "[stderr] " + stderr : ""));
                    else
                        resolve("Command started successfully (running in background, PID: " + child.pid + ").");
                }, 3000);
                child.on("close", (code) => {
                    clearTimeout(timer);
                    const result = stdout + (stderr ? nl + "[stderr] " + stderr : "");
                    if (!result.trim())
                        resolve("Command completed (exit: " + code + ").");
                    else
                        resolve(result + nl + "[exit: " + code + "]");
                });
    ...
    executors.list_directory = async (args) => {
        ...
        try {
            return readdirSync(path).join(nl);
        }
        catch (e) {
            return "Error: " + e;
        }
