...
function isTowelieBot(bot: { name: string; title?: string }): boolean {
...
const server = createServer(async (req, res) => {
  ...
  try {
    ...
    if (m && method === "PATCH") {
      ...
      for (const key of ["name", "title", "description", "notifications", "modelSelection", "unread", "computer", "deviceId", "color", "mascotExpression", "pinned", "hidden", "pythonEnabled", "compressedComms", "towelieBehavior", "engineEnabled", "startOptions"] as const) {
    ...
    if (m && method === "POST") {
      ...
      if (bot.startOptions?.autoRecall) {
        ...
        if (existsSync(memDir)) {
          ...
          if (keys.length > 0) store.appendMessage(bot.threadId, { role: "bot", kind: "text", text: "Memory recalled: " + keys.join(", ") });
        }
      }
    ...
    if ((method === "PUT" || method === "PATCH") && path === "/api/config") {
      ...
      for (const key of ["xai", "composio", "box", "ollama", "ollamaWorkstation", "ollamaMjLaptop", "ollamaCloud", "profile", "settingsPassword", "favoriteModels"] as const) {
    ...
    if (method === "POST" && path === "/api/engines/build") {
      ...
      try {
        ...
        const searchRes = await fetch(
          "https://en.wikipedia.org/w/api.php?action=query&list=search&srsearch=" + encodeURIComponent(subject) + "&format=json&srlimit=1",
        ...
        const contentRes = await fetch(
          "https://en.wikipedia.org/w/api.php?action=query&prop=extracts&explaintext=true&titles=" + encodeURIComponent(wikiTitle) + "&format=json",
          { signal: AbortSignal.timeout(10000) }
        );
        const contentData: any = await contentRes.json();
        const pages = contentData.query?.pages ?? {};
        const fullText = (Object.values(pages)[0] as any)?.extract || summary;
        // Split into sections
        ...
        if (enhance) {
          ...
          try {
            ...
            if (llmRes.ok) {
              ...
              if (enhanced && enhanced.length > 500) engine = enhanced;
            }
    ...
    if (method === "POST" && path === "/api/devices/scan") {
      const results: Array<{name: string; type: string; host: string; port?: number; adbDeviceId?: string}> = [];
      // 1. Scan ADB devices
      try {
      ...
      try {
        ...
        for (const line of arpOut.split(String.fromCharCode(10))) {
          ...
          if (m && m[1] !== "224.0.0.22" && !m[1].startsWith("239.")) {
            ...
            if (!existing) results.push({ name: "Network Device (" + m[1] + ")", type: "ssh", host: m[1], port: 22 });
          }
        }
      } catch {}
