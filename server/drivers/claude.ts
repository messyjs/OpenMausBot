...
const DENY_TIMEOUT_NOTE =
  "Towelie Bot: nobody answered this permission request in time. Skip this action and finish what you can without it.";
  ...
  return {
    ...
    close() {
      for (const p of [...pending.values()]) {
        if (p.ask.kind === "question") p.finish("answer", "Towelie Bot: the turn is ending — wrap up.", "shutdown");
        else p.finish("deny", "Towelie Bot: the turn ended", "shutdown");
      }
      try {
        server.close();
      } catch {}
