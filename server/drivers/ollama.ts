...
(async () => {
  try {
    ...
    let finalText = "";
    let finalUsage: { input: number; output: number } | null = null;
    ...
    for (let round = 0; round <= MAX_TOOL_ROUNDS; round++) {
      ...
      for (const m of (matches||[])) {
        try { const j=JSON.parse(m.replace(new RegExp(tcS+"|"+tcE,"g"),"").trim()); calls.push({name:j.name||"",args:j.arguments||{}}); } catch {}
      }
    ...
    if (finalText.trim()) emit({ ...base(threadId, turnId), type: "item.completed", itemType: "assistant_text", text: finalText });
    if (finalUsage) emit({ ...base(threadId, turnId), type: "thread.token-usage.updated", ...finalUsage });
